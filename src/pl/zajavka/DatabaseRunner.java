...
public class DatabaseRunner {
    ...
    private static final String SQL_INSERT =
            "INSERT INTO TODO (NAME, DESCRIPTION, DEADLINE, PRIORITY) VALUES (?, ?, ?, ?);";
    private static final String SQL_UPDATE =
            "UPDATE TODO SET DESCRIPTION = ?, DEADLINE = ?, PRIORITY = ? WHERE NAME = ?;";
    private static final String SQL_READ_WHERE = "SELECT * FROM TODO WHERE NAME = ?;";
    private static final String SQL_READ_ALL = "SELECT * FROM TODO ORDER BY ?1 ?2";
    private static final String SQL_GROUPED =
            "SELECT DATE(deadline) AS DATE, ARRAY_AGG(name) AS TASKS FROM TODO GROUP BY DATE(deadline) ORDER BY DATE;";
    private static final String SQL_DELETE = "DELETE FROM TODO WHERE NAME = ?;";
    private static final String SQL_DELETE_ALL = "DELETE FROM TODO;";
    private static final String SQL_COMPLETED = "UPDATE TODO SET STATUS = ? WHERE NAME = ?;";
    ...
    private void runAdd(final Command command) {
        ...
        try (
            ...
            System.out.printf("Run: [%s] successfully, inserted: [%s]%n", command.getType(), count);
    ...
    private void runUpdate(final Command command) {
        ...
        try (
            ...
            int count = statement.executeUpdate();
            System.out.printf("Run: [%s] successfully, updated: [%s]%n", command.getType(), count);
        } catch (SQLException e) {
    ...
    private void runRead(final Command command) {
        ...
        try (
            ...
            try (ResultSet resultSet = statement.executeQuery()) {
                List<ToDoItem> readItems = mapToToDo(resultSet);
                print(readItems);
                System.out.printf("Run: [%s] successfully, updated: [%s]%n", command.getType(), readItems.size());
            }
        } catch (SQLException e) {
            System.err.printf("[%s] data error. Message: [%s]%n", command.getType(), e.getMessage());
        }
    ...
    private void runGrouped(final Command command) {
        ...
        try (
                Connection connection = DriverManager.getConnection(URL, USERNAME, PASSWORD);
                PreparedStatement statement = connection.prepareStatement(SQL_GROUPED)
        ) {
            try (ResultSet resultSet = statement.executeQuery()) {
                var grouped = mapToGrouped(resultSet);
                print(grouped);
                System.out.printf("Run: [%s] successfully, updated: [%s]%n", command.getType(), grouped.size());
            }
        } catch (SQLException e) {
            System.err.printf("[%s] data error. Message: [%s]%n", command.getType(), e.getMessage());
        }
    ...
    private List<ToDoItem> mapToToDo(ResultSet resultSet) throws SQLException {
    ...
    private void runDelete(final Command command) {
        ...
        try (
            ...
            statement.setString(1, command.getToDoItem().getName());
            int count = statement.executeUpdate();
            System.out.printf("Run: [%s] successfully, deleted: [%s]%n", command.getType(), count);
        } catch (SQLException e) {
            System.err.printf("[%s] data error. Message: [%s]%n", command.getType(), e.getMessage());
        }
    }
